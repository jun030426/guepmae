...
const REPORT_SCHEMA = z.object({
  ...
  location: z.object({
    ...
    school: z.string().describe('학군 — 학군 데이터는 제공되지 않음. 반드시 "공개 데이터로 학군 정보는 확인되지 않아 직접 확인이 필요합니다"로만 작성. 학교명·배정 정보를 절대 지어내지 말 것.'),
...
3. 제공되지 않은 정보(학군·학교명·지하철역 거리·권리관계·개발 호재 등)는 절대 지어내지 마세요. 모르면 "공개 데이터로 확인되지 않음 — 직접 확인 필요"로 표기하세요.
